...
const char* R_ParseErrorStr(R_ParseError err)
{
    switch (err)
    {
        ...
        case R_ParseError::GainInvalid:
            return "Gain invalid (should be a number ending with 'db')";
    }
...
void R_ApplyGain(AudioFrame<SampleT>& frame, float scalar_gain)
{
    frame.left  = (SampleT)((float)frame.left * scalar_gain);
    frame.right = (SampleT)((float)frame.right * scalar_gain);
}
...
Audio options:
  ...
  --gain <amount>db            Apply gain to the output.
